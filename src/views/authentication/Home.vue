...
import { uploadFaceFile, faceCheck, faceActionSequence, getFaceType, faceActionSequenceG } from '@/api/face'
  ...
  methods: {
    ...
    getSequence() {
      getFaceType().then(res => { // 获取人脸类型
        if (res.code === '00') {
          this.faceType = res.context.faceType
          if (this.faceType === '3') {
            if (this.show !== '0') {
              if (this.$route.query.check && this.$route.query.check === 'checked') return
              this.$router.push('./show')
            }
            faceActionSequenceG().then(res => {
              if (res.code === '00') {
    ...
    validateVideo() {
      ...
      this.$dialog.confirm({
        ...
        uploadFaceFile(this.faceType, formData).then(res => {
          if (res.code === '00') {
            this.fileId = this.faceType === '3' ? res.context.fieldId : res.context.fileId
            faceCheck(this.faceType, {
              'sequence': this.sequence,
              'faceFileId': this.fileId,
              'tokenId': this.tokenId,
              'userType': this.faceType === '3' ? '4' : '2'
            }).then(res => {
              if (res.code === '00') {
